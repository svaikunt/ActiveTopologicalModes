...
MatrixElementRectangleAllLinksRotJ= Compile[{{i, _Integer}, {j, _Integer}, {s1, _Real}, {s2, _Real}, {t1, _Real},{b1, _Real},{Ea,_Real},{Nt,_Real},{Q,_Complex}},
		If [EvenQ[i],
			If[j-1 == i,   s1 (*Even to Odd *), 
				If[j==i-2,   t1 Exp[-I Q], (*m to m+1*)
			If[j==i+2, t1 Exp[I Q] ,If[j==i,-2*t1-1,0]]]],(*m+1 to m*)(*Odd to Even*)
			If [
				j == i+2, s2 Exp[I  Q] ,  
				If[j==i-2,  s2   Exp[-I Q], 
					If[j==i+1,  -b1 Exp[I  Q] , 
...
(*Wtemp=SparseArray[{{i_,j_}->MatrixElementRectangleAllLinksSqJ[i, j, s1, s2,t1, b1, b2,Ea,El,Nt,Q]}];*)
    ...
    Wtemp[[1]][[2]]=0;
    Wtemp[[2]][[1]]=0;
    Wtemp[[Nsites]][[Nsites]]=0;
    Wtemp[[2]][[2]]=0;
    Wtemp[[Nsites-1]][[Nsites-1]]=0;*)
